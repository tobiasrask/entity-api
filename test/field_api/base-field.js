    ...
    it('Should lock field updates', (done) => {
      ...
      for (var i = 0; i < numProbes; i++) {
        ...
        if (instance.get() != probe.values.fieldValue2Probe) {
          errors.push(new Error(`Unable to force update protected value: ${instance.get()},
            expecting: ${probe.values.fieldValue2Probe}`))
        }
        ...
        if (instance.get() != probe.values.fieldValueProbe) {
          errors.push(new Error(`Unable to update unprotected value: ${instance.get()},
            expecting: ${probe.values.fieldValueProbe}.`))
        }
      }
