  ...
  describe('Field construction', () => {
    it('Should construct with random field probes', (done) => {
      ...
      for (var i = 0; i < numProbes; i++) {
        ...
        valueTests.map((testCase, _index) => {

          let result = instance.setValue(testCase['value'])

          if (testCase.shouldAllow && !result) {
            errors.push(new Error(`It didn't allow value to be type of: ${typeof testCase['value']}`))

          } else if (!testCase.shouldAllow && result) {
            errors.push(new Error(`Unexpected value was allowed: ${typeof testCase['value']}`))

          } else {
            // Test expected value
            if (instance.getValue() != testCase.expectedValue) {
              errors.push(new Error(`Got: ${instance.getValue()}, expected: ${testCase.expectedValue},
