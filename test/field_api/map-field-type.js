...
import { MapFieldType } from "./../../src/index"
...
class TestUtils extends Utils {
  ...
  */
  static createProbe(variables) {
    ...
    class ProbeFieldType extends MapFieldType {
      ...
      for (var i = 0; i < numProbes; i++) {
        ...
        valueTests.map((testCase, index) => {
          ...
          if (testCase.shouldAllow && !result) {
            errors.push(new Error("Test case #" + index + " failed. Didn't allow value to be type of: " + typeof testCase['value']) + ", original value: " + testCase['value']);
